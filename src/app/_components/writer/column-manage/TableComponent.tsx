...
const TableComponent = ({ dataSource }: { dataSource: OrderBuyer[] }) => {
        ...
        <span className={"text-3.5 font-400"}>
          {value ? (
            <span className={"flex items-center"}>
              <span className={"bg-#1DB48D h-1.5 w-1.5 rounded-full"}></span>
              <span className={"ml-2"}>订阅中</span>
            </span>
          ) : (
            <span className={"flex items-center"}>
              <span className={"bg-#BFBFBF h-1.5 w-1.5 rounded-full"}></span>
  ...
  const onChange: TableProps<OrderBuyer>["onChange"] = (
    pagination,
    filters,
    sorter,
    extra,
