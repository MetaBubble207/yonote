...
export const Article = () => {
    ...
    useEffect(() => {
        if (postData?.content  && postData.content.length > 176) {
            setPostContent(postData.content = postData.content.substring(0, 176) + "...");
        } else {
        ...
        <div className="w-85.75 h-129.5005 bg-[#ffffff] ml-4">
            ...
            <div className="flex">                        
                <div className="flex items-center w-full h-19.375">
                    {/* 作者头像与名称 */}
                    <Image src={user?.avatar?user?.avatar:"/images/recommend/cover.svg"} alt={"usericon"} width={49} height={49} className=" w-12.25 h-12.25 shrink-0 ml-5.25 mt-7.125"/>             
