...
const Article = () => {
        ...
        const {data: postData, isLoading: isPostDataLoading} = api.post.getById.useQuery({
            id: columnId,
            chapter: chapter
        });
        ...
        if (isUserLoading || isPostDataLoading || isReadLoading || isLikeLoading) return <div
            className={"h-screen w-full flex items-center justify-center"}><Loading/></div>
        return (
            <div className="min-h-screen bg-[#999999] flex justify-center items-center">
                {isScreenshot &&
                    <div className={"absolute h-screen w-full flex items-center justify-center z-100"}><Loading/></div>}
                {pngUrl !== ""
                    ?
                    <div className={"relative w-88 h-140"}>
                        <Image src={pngUrl} alt={"poster"} fill quality={100}/>
                    </div>
                    :
                    <div ref={png} className="w-full h-129.5005 bg-[#ffffff] mx-4">
                        {/* 顶部作者信息 */}
                        <div className="flex">
                            <div className="flex items-center w-full h-19.375">
                                ...
                                <div className="flex flex-col h-20">
                                    ...
                                    <div className="flex items-center ml-2.25">
                                        <div
                                            className="text-[#252525] font-D-DIN text-3.5 font-700 lh-6">{likeCount}</div>
                                        <div className="w-10 text-[#999] text-3 font-400 lh-6 ml-1.25 mt-0.5">点赞</div>
                                        <div
                                            className="text-[#252525] font-D-DIN text-3.5 font-700 lh-6 ml-1">{readCount}</div>
                                        <div className="w-10 text-[#999] text-3 font-400 lh-6 ml-1.25 mt-0.5">浏览</div>
                                    </div>
                                </div>
                            </div>
                            <Image src={"/images/poster/wholeLogo.svg"} alt="wholeLogo" width={2} height={2}
                                   className="w-16 h-12 mt-7 mr-5"></Image>
                        </div>
                        ...
                        <div className="relative w-72.462 h-43.25 mt-4 ml-4.5">
                            <Image src={"/images/poster/title2.svg"} alt="title2" width={2} height={2}
                            ...
                            <div className="w-40 mt-2">
                                <Button type={'link'} size={'small'} onClick={handleScreenshotClick}>
                                    <div className="flex items-center">
                                        ...
                                        <div className=" text-[#666] text-2.5 font-500 ml-1.5">点击生成图片后长按保存</div>
                                    </div>
                                </Button>
